...
print("                             Read data")
print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
df = spark.read.option("header", "true").csv("ProjectData_ProjectLeader_Mohamed_ElAlem.csv")
df.show(n=5)
print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
...
dfnew = df.withColumn("Largest Property Use Type - Gross Floor Area (ft)",
    ...
    .withColumn("Electricity Use - Grid Purchase (kBtu)",
                df["Electricity Use - Grid Purchase (kBtu)"].cast(FloatType())) \
    .withColumn("Total GHG Emissions (Metric Tons CO2e)",
                df["Total GHG Emissions (Metric Tons CO2e)"].cast(FloatType())) \
    .withColumn("Direct GHG Emissions (Metric Tons CO2e)",
                df["Direct GHG Emissions (Metric Tons CO2e)"].cast(FloatType())) \
    .withColumn("Indirect GHG Emissions (Metric Tons CO2e)",
                df["Indirect GHG Emissions (Metric Tons CO2e)"].cast(FloatType())) \
    .withColumn("Weather Normalized Site Electricity (kWh)",
                df["Weather Normalized Site Electricity (kWh)"].cast(FloatType())) \
    .withColumn("Weather Normalized Site Natural Gas Use (therms)",
                df["Weather Normalized Site Natural Gas Use (therms)"].cast(FloatType())) \
...
print("                             Drop columns")
print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
delist = ["Address 2", "2nd Largest Property Use Type", "2nd Largest Property Use - Gross Floor Area (ft)",
...
group_data.agg({'Primary Property Type - Self Selected': 'count', 'ENERGY STAR Score': 'sum'}).sort(
    'count(Primary Property Type - Self Selected)', ascending=False).show(truncate=False, n=100)

filtered_by_property_office = df3.filter(F.col('Primary Property Type - Self Selected') == 'Office').select(
    F.col('ENERGY STAR Score').alias('ENERGY STAR Score_property_office'))
x1 = filtered_by_property_office.toPandas()["ENERGY STAR Score_property_office"].values.tolist()
filtered_by_property_Multifamily = df3.filter(
    F.col('Primary Property Type - Self Selected') == 'Multifamily Housing').select(
    F.col('ENERGY STAR Score').alias('ENERGY STAR Score_property_Multifamily'))
x2 = filtered_by_property_Multifamily.toPandas()["ENERGY STAR Score_property_Multifamily"].values.tolist()
filtered_by_property_NonRefrigeratedWarehouse = df3.filter(
    F.col('Primary Property Type - Self Selected') == 'Non-Refrigerated Warehouse').select(
    F.col('ENERGY STAR Score').alias('ENERGY STAR Score_property_Non-Refrigerated Warehouse'))
x3 = filtered_by_property_NonRefrigeratedWarehouse.toPandas()[
    "ENERGY STAR Score_property_Non-Refrigerated Warehouse"].values.tolist()

filtered_by_property_Hotel = df3.filter(F.col('Primary Property Type - Self Selected') == 'Hotel').select(
    F.col('ENERGY STAR Score').alias('ENERGY STAR Score_property_Hotel'))
...
property_hotel_ele_internsity = df3.filter(F.col('Primary Property Type - Self Selected') == 'Hotel').select(
    F.col('Weather Normalized Site Electricity Intensity (kWh/ft)').alias('property_hotel_ele_internsity'))
...
property_hotel_gross_floor = df3.filter(F.col('Primary Property Type - Self Selected') == 'Hotel').select(
    F.col('Largest Property Use Type - Gross Floor Area (ft)').alias('property_hotel_gross_floor'))
...
property_hotel_Electricity_Use = df3.filter(F.col('Primary Property Type - Self Selected') == 'Hotel').select(
    F.col('Electricity Use - Grid Purchase (kBtu)').alias('property_hotel_Electricity_Use(kWh)'))
property_hotel_Electricity_Use_df = property_hotel_Electricity_Use.withColumn('property_hotel_Electricity_Use(kWh)',
                                                                              property_hotel_Electricity_Use[
                                                                                  'property_hotel_Electricity_Use(kWh)'] / 3412.14)
...
property_Multifamily_Housing_Electricity_Intensity = df3.filter(
    F.col('Primary Property Type - Self Selected') == 'Multifamily Housing').select(
    F.col('Weather Normalized Site Electricity Intensity (kWh/ft)').alias(
        'property_Multiy_famliy_Electricity_Intensity'))
...
property_Multifamily_Housing_gross_floor = df3.filter(
    F.col('Primary Property Type - Self Selected') == 'Multifamily Housing').select(
    F.col('Largest Property Use Type - Gross Floor Area (ft)').alias('property_Multiy_famliy_gross_floor'))
...
property_Multifamily_Electricity_Use = df3.filter(
    F.col('Primary Property Type - Self Selected') == 'Multifamily Housing').select(
    F.col('Electricity Use - Grid Purchase (kBtu)').alias('property_Multiy_famliy_Electricity_Use'))
property_Multifamily_Electricity_Use_df = property_Multifamily_Electricity_Use.withColumn(
    'property_Multifamily_Electricity_Use(kWh)',
    property_Multifamily_Electricity_Use['property_Multiy_famliy_Electricity_Use'] / 3412.14)
...
property_office_Electricity_Intensity = df3.filter(F.col('Primary Property Type - Self Selected') == 'Office').select(
    F.col('Weather Normalized Site Electricity Intensity (kWh/ft)').alias('property_Office_Electricity_Intensity'))
...
gross_floor = df3.filter(F.col('Primary Property Type - Self Selected') == 'Office').select(
    F.col('Largest Property Use Type - Gross Floor Area (ft)').alias('Office_gross_floor'))
gross_floor.sort('Office_gross_floor')
x12 = gross_floor.toPandas()

ele_use = df3.filter(F.col('Primary Property Type - Self Selected') == 'Office').select(
    F.col('Electricity Use - Grid Purchase (kBtu)').alias('office_Electricity_Use(kWh)'))
